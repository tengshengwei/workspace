...
import com.karuite.weixin.util.GameUtil;
...
public class GameService {
	 ...
	 * @return
	 */
	public static String process(String openId, String content) {
		// 获取用户最近一次创建的游戏
		Game game = MySQLUtil.getLastGame(openId);
		// 本局游戏的正确答案
		...
		// 查询本局游戏的所有回合
		List<GameRound> roundList = MySQLUtil.findAllRoundByGameId(gameId);
		// 遍历游戏回合，组装给用户回复的消息
	...
	public static String getUserScore(HttpServletRequest request, String openId) {
		StringBuffer buffer = new StringBuffer();
		HashMap<Integer, Integer> scoreMap = MySQLUtil.getScoreByOpenId(openId);
