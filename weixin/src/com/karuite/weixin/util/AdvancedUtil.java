...
import com.karuite.weixin.util.pojo.WeixinQRCode;
import com.karuite.weixin.util.pojo.WeixinUserInfo;
import com.karuite.weixin.util.pojo.WeixinUserList;
...
public class AdvancedUtil {
	...
	public static String getQRCode(String ticket, String savePath) {
		...
		try {
            ...
            response.addHeader("Content-Disposition", "attachment;filename="+System.currentTimeMillis()+".jpg");// Content-Disposition中指定的类型是文件的扩展名，并且弹出的下载对话框中的文件类型图片是按照文件的扩展名显示的，点保存后，文件以filename的值命名，保存类型以Content中设置的为准。注意：在设置Content-Disposition头字段之前，一定要设置Content-Type头字段。
